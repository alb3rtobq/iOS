...
class ItemsListViewController: UIViewController {
    ...
    @IBAction func didChangeSegment(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0 {
            items = items.sorted(by: { $1.inKartProduct != $0.inKartProduct } )
...
extension ItemsListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        refreshAlert.addAction(UIAlertAction(title: "OK", style: .default, handler: { (action: UIAlertAction!) in
            self.items[indexPath.row].inKartProduct = statusProduct
            
            for item in self.items {
                
                if item.inKartProduct == true { self.priorityMenuSegmentedControl.setEnabled(true, forSegmentAt: 0) }
                    else {
